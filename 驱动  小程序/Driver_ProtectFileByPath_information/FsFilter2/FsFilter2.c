...
// TODO - List all of the requests to filter.
	...
	{ IRP_MJ_CREATE,
      ...
      NULL },
#if 0 
	{ IRP_MJ_SET_INFORMATION,    //////---------------------------------发出一个设置请求，表示这个文件将被删除。
	0,
	FsFilter2PreOperation,
	NULL },
    ...
    )
{
	...
	//获取创建文件时候的安全权限。
	pSecurityContext = Data->Iopb->Parameters.Create.SecurityContext;

	if (pSecurityContext && (pSecurityContext->DesiredAccess&DELETE))
	{
		status = FltGetFileNameInformation(Data, FLT_FILE_NAME_NORMALIZED, &fltName);
		if (!NT_SUCCESS(status))
		{
			if (status != STATUS_OBJECT_PATH_NOT_FOUND)
				DbgPrint(" gxb FsFilter1!" __FUNCTION__ ": FltGetFileNameInformation failed with code:%08x\n", status);

			return FLT_PREOP_SUCCESS_WITH_CALLBACK;
		}
		KdPrint(("----- gxb the file name  %ws \r\n", fltName->Name.Buffer));
		//打印文件的路径信息
		if (wcsstr(fltName->Name.Buffer, L"SCBYDL") != NULL)
		{
